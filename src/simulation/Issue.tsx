...
export const issueStep = (
  ...
  if (instruction.type === 'memory') {
    ...
    let newTransmitFlags = {
      ...currentState.transmitFlags,
      instructionQueueToLoadStoreBuffers: true,
    }

    const getMemoryOperand = (
      regIndex: number
    ): {
      value?: number
      ...
      newTransmitFlags = {
        ...newTransmitFlags,
        registerFileToLoadStoreBuffers: true,
      }
    ...
    if (instruction.opcode === 'ld') {
      const freeIndex = loadBuffers.findIndex(
      ...
      loadBuffers[freeIndex] =
        baseOperand.value !== undefined
          ? {
              isEmpty: false,
              isLoading: false,
              isReady: true,
    ...
    if (instruction.opcode === 'st') {
      const freeIndex = storeBuffers.findIndex(
        (buffer) => buffer.isEmpty
      )
