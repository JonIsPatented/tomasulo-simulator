...
export type WaitingArgument =
  | StationWaitingArgument
  | LoadWaitingArgument

export type ReservationStationArgument<T> =
  | ReadyArgument<T>
  | WaitingArgument
...
export type ReservationStationData<T> =
  | ReadyStation<T>
  | WaitingStation<T>
  | EmptyStation
...
export type ActiveBus<T> =
  | ActiveLoadSignalBus<T>
  | ActiveOperationSignalBus<T>
...
export type FunctionUnitData<T> =
  | ActiveFunctionUnit<T>
  | EmptyFunctionUnit
...
export interface SimulatorData {
  ...
  // the memory unit itself, stored as a Map of addresses
  // to values in those addresses
  memoryUnit: Map<number, number>
...
export class Simulation {
  ...
  public readonly unsubscribe = (id: string): boolean => {
    if (
      !this.subscribers.some((subscriber) => subscriber.id === id)
    ) {
      return false
    }
  ...
  private currentState: SimulatorData = {
    // TODO
    ...defaultState(),
    registerFile: [
  ...
  public readonly adjustInstructionDurations = (
    ...
    InstructionDurations,
    'NEGATIVE_DURATION' | 'ALREADY_RUNNING'
  > => {
    if (this.currentState.running) return failure('ALREADY_RUNNING')
    const newDurations = adjustment(
      this.currentState.cyclesPerInstruction
    )
  ...
  > => {
    ...
    const result = assembleProgram(
      program,
      this.currentState.registerFile.length
    )
...
const defaultState = (): SimulatorData => ({
  ...
  memoryUnit: new Map<number, number>(),
  clockRate: 4,
  instructionQueue: [],
  transmitFlags: {
    registerFileToReservationStations: false,
