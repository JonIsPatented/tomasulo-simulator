    ...
    <svg className='pointer-events-none absolute top-0 left-0 h-full w-full'>
      ...
      <Wire
        from={{
          ...
          x:
            (left(loadStoreBuffers).x + bottom(loadStoreBuffers).x) /
            2,
        }}
        to={{
          ...top(commonDataBus),
        ...
        active={transmitFlags.loadBuffersToCommonDataBus}
      />
      ...
      <Wire
        from={{
          ...top(commonDataBus),
          x:
            (left(reservationStations).x + left(functionUnits).x) / 2,
        }}
        to={{
      ...
      <Wire
        from={{
          ...
          x:
            (right(loadStoreBuffers).x + top(loadStoreBuffers).x) / 2,
        }}
        to={{
          ...bottom(memoryUnit),
          x:
